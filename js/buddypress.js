...
/* 		alert(1111); */
		...
		var category = jq('input[name="category"]').val();
		
		
		var get_more_traffic = jq('#get_more_traffic').val();
	
		
		var your_call_to_action = jq("input#your_call_to_action").val();
		
		if(content == "" && get_more_traffic == "" && your_call_to_action == ""){
			/* alert(1); */
			jQuery("#testcase2").show();
		}else if(content == "" && get_more_traffic == "" && your_call_to_action != ""){
		/* alert(2); */
		jQuery("#testcase2").show();
		...
		jQuery("#testcase2").show();
			...
			if ( response[0] + response[1] === '-1' ) {
				...
				if ( 0 === jq('ul.activity-list').length ) {
					jq('div.error').slideUp("fast").remove();
					jq('#message').slideUp("fast").remove();
				...
				var randomColor = function() {
				    ...
				    //alert(safeColors[rand()]);
				    return "#"+r+g+b;
				};

				jQuery('#activity-stream .activity').each(function() {
			    	/* jQuery(this).css('border','2px');
			    	jQuery(this).css("border-style","solid");
			        jQuery(this).css('border-color',randomColor());
					jQuery(this).find("div.show-all").hide(); */
			    });
				/* kleo added */
				...
				if ( 0 !== jq('#latest-update').length ) {
					...
					jq('#latest-update').slideUp("fast",function(){
						jq('#latest-update').html( u );
						jq('#latest-update').slideDown("fast");

						/*jq('.post_msg').css('display','block');*/
						jq('#whats-new-form').css('display','none');
			...
			jq( '#whats-new-content' ).removeClass( 'active' );
				...
				var link = 'http://hive.tradesecrets.biz/wp-content/themes/kleo-child/ajax.php';
		
				jq.ajax({
					type:'post',
					url:link,
					data:{action:'send_mail_to_new_post',industry:category},
					success : function(){
						/* if(jq("#whats-new").val()){
							jq('#whats-new-form').css('display','none');
						} */
						jq("#message").css('display','none');
						jq('#whats-new-form').css('display','none');
		...
		var post_length = jq('ul.activity-list li').length;
	/* 	console.log(post_length); */
		if(post_length == 0){
	    	/* alert(post_length); */
			jQuery('.first_post').show();
			jQuery('.first_post').html('Post advice for your industry and plug your business.');
	    	jQuery('.rm').replaceWith('');
			jQuery(".ajax_advice_post").show();
			jQuery(".post-an-advice-section").show();
			jQuery("#activity-stream li").removeClass('other_industry_check');
			/* location.reload(); */ 
	    }/*else if(post_length >= 1){
			jQuery('.first_post').show();
			jQuery('.first_post').html('Post advice for your industry and plug your business.');
		...
		if ( target.hasClass('fav') || target.hasClass('unfav') ) {
			...
			function(response) {
				...
				if ( 'activity-favorites' === jq( '.item-list-tabs li.selected').attr('id') ) {
					target.closest( '.activity-item' ).slideUp( "fast" );
				}
			});
		...
		if ( target.hasClass('delete-activity') ) {
			...
			function(response) {
				...
				if ( response[0] + response[1] === '-1' ) {
					...
					li.children('#message').hide().fadeIn("fast");
				} else {
					li.slideUp("fast");
					location.reload();
					// reset vars to get newest activities
				...
				var count1 = -1;
			jq('ul.activity-list li').each(function(){
			...
			if(count1 == 0){
				/* alert("c"+count1); */
				jq('ul.activity-list').replaceWith('<div id="message" class="info login_div"><p><strong>'+category+' business advice.</strong><br>New industry! No advice posted.<br>Be the first to advise your '+category+' friends!<br>Click <a class="login_no_post">here</a>.</p></div><script type="text/javascript">jQuery(document).ready(function(){jQuery(".login_no_post").click(function(){jQuery(".background_btn").toggleClass("post_an_advice_btn");});});</script>');
				jQuery('.first_post').html('');
			}/* else if(count1 == 1){ */
				/* alert("count"+count1); */
			/* jQuery('.first_post').show();
				jQuery('.first_post').html('Post advice for your industry and plug your business.');
 ...
 */
		...
		if ( target.parent().hasClass('load-more') ) {
			...
			function(response)
			{
				...
				jq('#buddypress ul.activity-list').append('<div class="loadmore-custom-li">'+response.contents+'</div>');
				...
				jQuery('.loadmore-custom-li li').each(function() {
					/* jQuery(this).css('border','2px');
					jQuery(this).css("border-style","solid");
					jQuery(this).css('border-color',randomColor());
					jQuery(this).find("div.show-all").hide(); */
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.hasClass('acomment-reply') || target.parent().hasClass('acomment-reply') ) {
			...
			if(!jQuery("#hidden_avatar_check").length){
				form.slideDown( 200 );
			} else {
		...
		/* comment reply */
		if ( target.hasClass('comment-reply') || target.parent().hasClass('comment-reply') ) {
			/* alert("Comment-reply"); */
			
			if ( target.parent().hasClass('comment-reply') ) {
				target = target.parent();
			}
			...
			a_id = ids[2];
			c_id = target.attr('href').substr( 10, target.attr('href').length );
			form = jq( '#ac_reply_form-' + a_id );
			/* alert(c_id); */
			jq("#ac_reply_form_submit-"+a_id).attr("data-id",c_id);
			form.css( 'display', 'none' );
			form.removeClass('root');
			jq('.ac-form').hide();
			...
			if ( ids[1] !== 'comment' ) {
				jq('#comment-' + c_id).append( form );
			} else {
				/* alert("Form_append"); */
				jq('#activity-' + a_id + ' .activity-comments ul li#acomment-'+c_id).after( form );
			}
			...
			jq('#activity-' + a_id + ' .activity-comments ul li#acomment-'+c_id).append( form );
			/* jq.scrollTo( form, 500, {
				offset:-100,
				easing:'swing'
			} ); */
			jq('#ac_reply_form-' + ids[2] + ' textarea').focus();
			...
			jq.post( ajaxurl, ajaxdata, function(response) {
				...
				if ( response[0] + response[1] === '-1' ) {
					...
					if ( show_all_a ) {
						...
						show_all_a.html( 'View more comments' ); 
					}
				}
		...
		if ( target.attr('name') === 'ac_reply_form_submit' ) {
			...
			if ( !form_parent.hasClass('activity-comments') ) {
				...
				/* alert("form-2"); */
				comment_id = jq("#ac_reply_form_submit-"+form_id[1]).attr("data-id");
			}
			...
			jq.post( ajaxurl, ajaxdata, function(response) {
				...
				if ( response[0] + response[1] === '-1' ) {
					...
					/* Increase the "Reply (X)" button count */
					new_count = Number( jq('#activity-' + form_id[1] + ' a.acomment-reply span').html() ) + 1;
					jq('#activity-' + form_id[1] + ' a.acomment-reply span').html( new_count );
...
function bp_legacy_theme_hide_comments() {
	...
	comments_divs.each( function() {
		if ( jq(this).children('ul').children('li').length < 2 ) {
			return;
		}
